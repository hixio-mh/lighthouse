...
from heavy JavaScript. That said, we expect to reduce the weighting of TTI and
likely remove it in future major Lighthouse versions.
...
### Overall, what motivated the changes to the performance core?

The [adjusted FCP curve](https://github.com/GoogleChrome/lighthouse/pull/12556)
will score a bit more strictly, but aligns with the current defacto threshold of
good.

The [curve for TBT](https://github.com/GoogleChrome/lighthouse/pull/12576) will
also score a bit more strictly, but with the new, more linear curve, it means
there's a larger range where improvements in the metric are rewarded with
improvements in the score. If your page currently scores poorly with TBT, the
new curve will be more responsive to changes as it incrementally improves.
